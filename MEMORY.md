...
- **Review is on-demand only** — `/basic-review` is the sole entry point (renamed from `/review` on 2026-09-23 so it no longer collides with Claude Code's own `/review`); done-skill never triggers it. `review_process.md` says _how_ a review runs, not _when_. Do not reintroduce "every implementation triggers a full review". (2026-08-02)
...
- **GitNexus is retired (owner decision, removed 2026-09-23 with optimizer v1.49.0)** — its skills, its policy doc under `agent_docs/` (offloaded from CLAUDE.md on 2026-07-26), the commit-time guard hook and the `AGENTS.md` policy mirror are gone; `AGENTS.md` is a pointer to CLAUDE.md. Do not re-add a GitNexus policy, skill or hook. The `.gitnexus` ignore entries left in `.dockerignore`, `.prettierignore` and `eslint.config.js` are inert; removing them is a quality-config edit for the owner. `.gitignore` no longer lists `.gitnexus/` — a leftover local index is deleted, never committed. (2026-09-23)
