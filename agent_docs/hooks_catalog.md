...
> **Tier-1 hooks** (already in `.claude/settings.json`): GitNexus read-only pre-commit guard (no auto-analyze), context budget guard (CLAUDE.md / MEMORY.md / SCRATCHPAD.md), quality-config guard (`deny` on Edit/Write of `eslint.config.js` / `.prettierrc.json` / `.prettierignore` — a red check is fixed in the code, not in the rule; it reports itself inert when `jq` is missing. `tsconfig.json` is deliberately **not** guarded: it carries build layout next to the strictness flags, so a guard there would block legitimate build work), SessionStart memory reminder.
...
> **MEMORY.md / SCRATCHPAD.md size warnings are no longer here** — since v1.18.0 they are part of the Tier-1 **context budget guard** in `.claude/settings.json`, which checks all three budgeted files (`CLAUDE.md` 14k, `MEMORY.md` 16k, `SCRATCHPAD.md` 8k) in a single `PostToolUse` hook and points at `agent_docs/context_budget.md`. Nothing to paste; adjust the thresholds there if this project needs different ones.
