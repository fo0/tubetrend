...
> **Tier-1 hooks** (already in `.claude/settings.json`): context budget guard (CLAUDE.md / MEMORY.md / SCRATCHPAD.md), quality-config guard (`deny` on Edit/Write of `eslint.config.js` / `.prettierrc.json` / `.prettierignore` — a red check is fixed in the code, not in the rule; it reports itself inert when `jq` is missing. `tsconfig.json` is deliberately **not** guarded: it carries build layout next to the strictness flags, so a guard there would block legitimate build work), SessionStart memory reminder.
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "cmd=$(jq -r '.tool_input.command // empty'); if echo \"$cmd\" | grep -q 'git push'; then ref=$(echo \"$cmd\" | grep -oE '\\b(main|master|develop|trunk)\\b' | head -1); [ -n \"$ref\" ] || ref=$(git -C \"$CLAUDE_PROJECT_DIR\" rev-parse --abbrev-ref HEAD 2>/dev/null); case \"$ref\" in main|master|develop|trunk) cd \"$CLAUDE_PROJECT_DIR\" && { npm run typecheck && npm run lint && npm run build; } || { echo 'BLOCKED: typecheck/lint/build failing — never push to main on red.' >&2; exit 2; };; esac; fi"
    }
  ]
}
```

Trigger: `PreToolUse`. Filled with TubeTrend's actual gate commands in the chain's own order (`npm run typecheck` → `npm run lint` → `npm run build` — no test runner configured yet; add the test command once Vitest lands). Requires `jq` — without it the guard never fires (see Notes). Heuristic and **err-safe**: a bare `git push` is resolved via the currently checked-out branch, and a false positive (e.g. a branch name containing `main`, or pushing a feature ref while `main` is checked out) merely runs the checks — it only blocks when they are red.
