...
│   │   ├── dashboard/                # Dashboard feature (components + hooks + services)
...
│   └── skills/                       # done / pr / basic-review / basic-sec-review / rollback / ci / stuck / beacon / scheduler / orca
...
Beyond those four, `dashboard` also owns a `components/` directory — six components extracted from `DashboardPage.tsx` in #471. It is the only feature module with one; the other four have no feature-local components and render through `src/shared/components/`.
