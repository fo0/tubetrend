...
Verified against the tree on 2026-09-15. Line counts come from `find src -name '*.ts*' | xargs wc -l | sort -rn`; re-run it before trusting a number here.

- **`InputSection.tsx` (~911 lines)** — the largest file in `src/`. Search form, autocomplete, search history, timeframe/max-results controls and their `localStorage` persistence in one component. Split candidates: a `useSearchHistory()` hook and a separate autocomplete dropdown component.
...
- **`channelService.ts` (~508 lines)** — crossed the 500-line bar in #486. Carries two near-identical cache layers (channel cache and autocomplete cache: `get*Cache` / `is*Expired` / `get*FromCache` / `save*ToCache`, ~90 lines) that differ only in TTL and storage key. The split that pays here is one generic TTL-cache helper both layers instantiate, which also makes the cache behavior testable without a network stub. Five exports have to survive it: `searchChannels`, `findChannelInfo`, `getVideosFromChannel` plus the two pure helpers `getChannelQueryType` and `extractChannelIdentifier`.
