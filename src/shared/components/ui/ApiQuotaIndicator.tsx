...
const groupHistoryByTimeBuckets = (
  history: QuotaHistoryEntry[],
  timeWindowMs: number,
...
const getBucketTimeParts = (
  bucket: { startTime: number; endTime: number },
  timeWindowMs: number,
...
export const ApiQuotaIndicator: React.FC = () => {
  ...
  const timeBuckets = useMemo(
    () => groupHistoryByTimeBuckets(history, timeWindow.windowMs),
    [history, timeWindow.windowMs],
  );
  const maxUnitsInBucket = useMemo(
    () => Math.max(...timeBuckets.map((b) => b.units), 1),
    [timeBuckets],
  );
            ...
            <div className="relative h-16">
              <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                    ...
                    points={timeBuckets
                      .map((bucket, i) => {
                        ...
                        const y =
                          height -
                          padding -
                          (bucket.units / maxUnitsInBucket) * (height - padding * 2);
                        return `${x},${y}`;
                      })
                      .join(" ")}
                    fill="none"
                ...
                {timeBuckets.map((bucket, i) => {
                  ...
                  const x = (i / (timeBuckets.length - 1)) * (width - padding * 2) + padding;
                  const y =
                    height - padding - (bucket.units / maxUnitsInBucket) * (height - padding * 2);
                  ...
                  <div key={index} className="flex-1 relative group/point">
                    {bucket.units > 0 && (
                      <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-1 px-2 py-1 bg-slate-800 border border-slate-600 rounded text-[10px] text-slate-300 whitespace-nowrap opacity-0 group-hover/point:opacity-100 transition-opacity pointer-events-none z-[100]">
                        ...
                        <div>
                          {bucket.units} {t("quota.units")} ({bucket.calls} {t("quota.calls")})
                        </div>
                      </div>
                    )}
                  </div>
